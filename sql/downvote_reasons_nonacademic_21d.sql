-- Q24974 — Top Downvote Reasons — Non-Academic — last 21 days
-- ============================================================
-- Prod Metabase: https://metabase-prod.penpencil.co/question/24974
-- Used by: daily_digest.py fmt_nonacademic (rows fetched via fetch_metabase_card(24974))
-- Time window: rolling 21d from CURRENT_DATE
-- Filters: type IN ('copilot_message','message'), rating=0, intent VIDEO_CO_PILOT,
--          category in ('non_academic','non-academic') — handles both spellings
...
and json_extract_scalar(output, '$.metadata[0].category_name') in ('non_academic', 'non-academic')
